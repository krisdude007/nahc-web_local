...
<?php foreach($products as $product) {
    if (empty($models[$product->id]->product_option_id)) {
            ...
            <table class="table table-hover" id="productform-<?= $product->id ?>-product_option_id">
                ...
                <?php if (empty($models[$product->id]->product_option_id)) { ?>
                    <tr class="radio" style="display: table-row;" role="button">
                        <td>
                            <label><input type="radio" name="ProductForm[<?= $product->id ?>][product_option_id]"
                                          value="0" <?= (0 == $models[$product->id]->product_option_id ? 'checked' : null) ?>>None</label>
                        </td>
                        <td>
                            &nbsp;
                        </td>
                ...
                <?php foreach ($product->productOptionGroups as $optionGroup => $optionSet) { ?>
                    ...
                    <?php foreach ($optionSet as $optionId => $option) {
                        ...
                        <tr class="radio" style="display: table-row;" role="button">
                            <td>
                                <label><input type="radio" name="ProductForm[<?= $product->id ?>][product_option_id]"
                                              value="<?= $optionId ?>" <?= ($optionId == $models[$product->id]->product_option_id ? 'checked' : null) ?>><?= $itemLabel[0] ?>
                                </label>
                            </td>
                            <td>
                                <?= $itemLabel[1] ?>
